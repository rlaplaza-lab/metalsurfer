...
`compare_adaptive_grid_ab.py` is an optional A/B comparison of `auto` vs
`adaptive_grid` vs `rolling_probe` (not part of the official runner and not a
quick demo). See the [configuration guide](https://metalsurfer.readthedocs.io/en/latest/guides/configuration.html)
site-generator section for production defaults.
