...
def enrich_detailed_dataset_geometry(
    df: pd.DataFrame,
    data_dir: str | Path | None = None,
) -> pd.DataFrame:
    """Backfill quaternion and z_fraction columns for detailed CSV exports.

    New metalsurfer releases write these fields via
    :meth:`PlacementDescriptor.to_row`. Older ``adsorption_energies_detailed.csv``
    files can be backfilled from ``ml_dataset.csv`` in the same results directory
    when that file is present.
    """
    if all(col in df.columns for col in _DETAILED_GEOMETRY_COLS):
        return df

    working = df.copy()
    missing = [col for col in _DETAILED_GEOMETRY_COLS if col not in working.columns]

    had_missing_quats = not all(col in df.columns for col in _DETAILED_GEOMETRY_COLS)
    ml_used = False

    if data_dir is not None:
        ml_path = Path(data_dir) / _DATASET_FILENAME
        if ml_path.is_file():
            ml_df = pd.read_csv(ml_path)
            merge_keys = [
                key
                for key in _ML_DATASET_MERGE_KEYS
                if key in working.columns and key in ml_df.columns
            ]
            enrich_cols = [col for col in missing if col in ml_df.columns]
            if merge_keys and enrich_cols:
                ml_sub = ml_df[merge_keys + enrich_cols].drop_duplicates(
                    subset=merge_keys
                )
                working = working.merge(ml_sub, on=merge_keys, how="left")
                ml_used = True
                n_filled = int(working[enrich_cols[0]].notna().sum())
                logger.info(
                    "Backfilled %s from %s (%d/%d rows matched on %s)",
                    ", ".join(enrich_cols),
                    ml_path.name,
                    n_filled,
                    len(working),
                ...
                missing = [
                    col for col in _DETAILED_GEOMETRY_COLS if col not in working.columns
                ]
    ...
    for col in missing:
        working[col] = defaults[col]
    for col in _DETAILED_GEOMETRY_COLS:
        if col in working.columns:
            working[col] = working[col].fillna(defaults[col])

    if had_missing_quats and not ml_used:
        logger.warning(
            "Missing quaternion columns in detailed CSV and no %s found under %s; "
            "using identity-quaternion defaults for offline feature extraction",
            _DATASET_FILENAME,
            data_dir,
        )
