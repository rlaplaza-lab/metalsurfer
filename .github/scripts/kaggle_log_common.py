...
that may contain secrets (Kaggle API tokens, KGAT_* auth strings, HuggingFace
``hf_`` tokens); keep the redaction/parsing logic here so the two call sites
stay in sync.
"""
...
_REDACT_PATTERNS = (
    ...
    # HuggingFace user access tokens (hf_...) and common env dumps.
    re.compile(r"hf_[A-Za-z0-9]+"),
    re.compile(
        r"(?i)(hf[_-]?token|hugging[_-]?face[_-]?hub[_-]?token|hu?ggingface.*token)"
        r"\s*[:=]\s*\S+"
    ),
)
