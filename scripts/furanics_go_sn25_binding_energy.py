#!/usr/bin/env python3
"""Compute binding energies of furanic molecules on graphene oxide with 25% Sn coverage.

Molecules: HMF, BHMF, BHMTHF, 5-MF, MFA, DMF, MTHFA, DMTHF.

Slab: Graphene oxide from Mouhat et al., Nature Commun. 2020 (citable-data) + 25% Sn adatoms.
GO layer is fully frozen. Uses metalsurfer deposit_adatoms for Sn coverage.
Data in data/GO.xyz and data/cell_parameters.dat.
...
def main():
    parser = argparse.ArgumentParser(
        description="Furanic molecules on GO+25% Sn with BO (100 placements, batches of 20)"
    )
    parser.add_argument(
        "--device",
    ...
    results_subdir = "furanics_go_sn25"
    results_dir = f"results_{results_subdir}"
    os.makedirs(results_dir, exist_ok=True)

    base_slab = _load_go_slab()
    logging.info("GO slab: %d atoms", len(base_slab.atoms))
    ...
    config = AdsorptionConfig(
        ...
        placement_mode="auto",
        top_layer_tolerance=2.0,  # Include O/H in top layer for placement on GO
        relax_top_layer=False,  # Freeze entire GO layer (base_slab_for_frozen)
        bo_enabled=True,
    ...
    logging.info("GO + 25%% Sn: %d atoms", len(slab.atoms))
    ...
    print(f"E_slab (GO+25% Sn) = {e_slab:.4f} eV")
    ...
    for smiles, name in MOLECULES:
        ...
        if results:
            ...
            print(
                f"Binding energy of {name} on GO+25% Sn: "
    ...
    print("\n" + "=" * 60)
    print("Binding energy summary (graphene oxide + 25% Sn)")
    print("=" * 60)
