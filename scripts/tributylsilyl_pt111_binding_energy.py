#!/usr/bin/env python3
"""Binding energy screening of tri-n-butylsilyl anion (R3Si−, no H on Si) on Pt(111) from mp-126.

``min_contact_ratio=0.5`` allows close Si–Pt approaches during relaxation.
...
def main() -> int:
    ...
    surface_type = "tributylsilyl_pt111"
    ...
    campaign = run_adsorption(
        slab=slab,
        molecules=[(smiles, "tributylsilyl")],
        config=config,
    ...
    print(
        campaign.format_summary(
            title="Binding energy summary (tri-n-butylsilyl anion / Pt(111))",
            results_dir=results_dir,
        )
    )
    return 0
