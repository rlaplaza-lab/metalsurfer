#!/usr/bin/env python3
"""Acetylene saturation on RuCu alloy (Ru host, 1/2 Cu) from mp-33 using metalsurfer.
...
def main():
    ...
    surface_type = "acetylene_rucu_saturation"
    results_dir = f"results_{surface_type}"

    # Same surface creation as ethane_ethene_acetylene_rucu_binding_energy.py (seed=42)
    ...
    if campaign.runs:
        print(
            campaign.format_completion(
                label="Acetylene saturation on RuCu(0001)",
                results_dir=results_dir,
            )
        )
    else:
